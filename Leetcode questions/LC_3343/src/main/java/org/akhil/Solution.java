...
//Time Complexity: O(10 * H * T) which simplifies to O(H * T), where H is halfLen (n/2) and T is the target sum (at most 45 * 9). The recursion explores states defined by (digit, leftover, target). The digit goes from 0 to 10, leftover from 0 to halfLen, and target from 0 to the total sum / 2. The loop iterates at most halfLen times in each state. With memoization, each state is computed only once.
//Space Complexity: O(H * T) - Dominant space is used by the mem array for memoization.
