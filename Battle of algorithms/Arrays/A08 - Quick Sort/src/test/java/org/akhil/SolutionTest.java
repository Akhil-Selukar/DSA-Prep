...
class SolutionTest {
    ...
    @Test
    @DisplayName("Test sorting a large array with many duplicates")
    void testSortLargeArrayWithManyDuplicates() {
        int[] arr = new int[LARGE_ARRAY_SIZE];
        Random rand = new Random();
        // Fill with a limited range of values to create many duplicates
        for (int i = 0; i < LARGE_ARRAY_SIZE; i++) {
            arr[i] = rand.nextInt(50); // Numbers between 0 and 49
        }
